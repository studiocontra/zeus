  ...
  <div class="wrap-feed">
    ...
    <Swiper
      ...
      <SwiperSlide
        ...
        @mouseenter="showShape(randomVal(), $event)"
        @mouseleave="hideShape">
        <div class="wrap-post">
          <div class="wrap-video">
            <video
              ...
              :data-ref="`player-${idx}`"></video>

            <div class="play" @click="playVideo(idx)">
...
export default {
  ...
  mounted() {
    this.players = this.data.map((post, idx) => ({
      [`player-${idx}`]: videojs(document.querySelector(`video[data-ref="player-${idx}"]`), {
        ...this.playerOptions,
        sources: [
          {
            poster: post.poster.url,
    ...
    this.players.map(player => {
      ...
      playerInstance.on('pause', () => {
        playerInstance.el_.parentElement.parentElement.classList.remove('playing');
      });
    })
  },
  methods: {
    ...
    showShape(rand, e) {
      ...
      this.currentShape = rand;
      e.stopPropagation();

      document.querySelector('.wrap-shapes').style.opacity = 1;
      document.querySelector('.wrap-shapes').style.left = `${left}px`;
    },
    hideShape() {
      document.querySelector('.wrap-shapes').style.opacity = 0;
    },
    playVideo(id) {
      const [targetPlayer] = this.players.filter(item => {
        const [key] = Object.keys(item);
        return key === `player-${id}`;
      });

      targetPlayer[`player-${id}`].play();
      document.querySelector(`video[data-ref="player-${id}"]`).parentElement.parentElement.parentElement.classList.add('playing');
    }
  },
  computed: {
    videoId() {
